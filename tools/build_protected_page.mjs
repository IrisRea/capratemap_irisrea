...
const properties = records
  .map((row, index) => ({
    ...
    address: row[col["所在地"]],
    reit: row[col["投資法人名"]],
    acquisitionPrice: row[col["取得価格"]],
    appraisal: row[col["鑑定評価額"]],
    acquisitionDate: row[col["取得時期"]],
    ...
    built: row[col["建築時期"]],
    siteArea: row[col["地積"]],
    totalFloorArea: row[col["延床面積"]],
    landOwnership: row[col["土地の所有形態"]],
    buildingOwnership: row[col["建物の所有形態"]],
...
const detailEntries = properties.map((item) => [
  item.id,
  {
    a: item.address,
    r: item.reit,
    g: item.acquisitionPrice,
    p: item.appraisal,
    o: item.acquisitionDate,
    c: item.capRate,
    d: item.discountRate,
    ...
    b: item.built,
    e: item.siteArea,
    f: item.totalFloorArea,
    l: item.landOwnership,
    h: item.buildingOwnership,
    m: item.rentableArea,
